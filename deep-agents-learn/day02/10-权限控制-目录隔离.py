...
# 第一次执行：测试allowed_dir目录的读、写权限-允许
result = deep_agent.invoke(
    input={
        "messages": [
            {"role": "user",
             "content": "读取/temp_data目录中的local.txt中的内容，并在该目录下创建一个lzx.txt文件，内容是：我叫罗志祥，我是时间管理大师"}
        ]
    },
    config={
...
print("=" * 88)
# 第二次执行：测试根目录除allowed_dir目录以外其他目录的写权限-拒绝
result2 = deep_agent.invoke(
    input={
        "messages": [
            {"role": "user", "content": "在根目录创建lyf.txt文件，内容是：我PC被抓了"}
        ]
    },
    config={
